...
for world in "${test_worlds[@]}"; do
    pilots=(
    "ros2 launch drone_driver neuralPilot.launch.py trace:=False out_dir:=${output_directory}$world/profiling/neural_$world network_path:=$model_name"
    "ros2 launch drone_driver expertPilot.launch.py trace:=False out_dir:=${output_directory}$world/profiling/expert_$world"
    )
    for pilot in "${pilots[@]}"; do
    
        # Executes the simulator
        tmux send-keys -t 0 "ros2 launch drone_driver as2_sim_circuit.launch.py world:=../worlds/$world.world yaw:=3.14" C-m
        sleep 5
