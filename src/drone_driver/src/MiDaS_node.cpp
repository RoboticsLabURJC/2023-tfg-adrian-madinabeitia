...
#define MODEL_NAME = "model-small-traced.pt"

static const std::string OPENCV_WINDOW = "Image window";

class Midas: public rclcpp::Node
{
...
public:
    MidasNode() : Node("midas_node"), device(torch::Device(torch::kCPU)) {
        
        image_sub_ = this->create_subscription<sensor_msgs::msg::Image>(
            '/drone0/sensor_measurements/frontal_camera/image_raw', 1, std::bind(&MidasNode::imageCallback, this, std::placeholders::_1));
        
        image_pub_ = this->create_publisher<sensor_msgs::msg::Image>("/depth_image", 1);
...
int main(int argc, char** argv) {
    ...
    // Cambia el nombre de tu nodo y utiliza la clase correspondiente para ROS2
    auto node = rclcpp::Node::make_shared("midas");

    // Crea una instancia de la clase Midas (asegúrate de que la clase y los encabezados estén disponibles)
    auto midas = std::make_shared<Midas>(node);

    // Ejecuta el bucle de eventos de ROS2 en lugar de ros::spin()
    rclcpp::spin(node);
